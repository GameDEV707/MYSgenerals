...
class PeerLink {
    constructor(playerId, host, peerId) {
        this.host = host;
        this.peerId = peerId;
        this.playerId = playerId;
    }
    pushSnapshot(s) { this.host.deliver(this.peerId, { m: "snapshot", data: s }); }
    pushEvent(e) { this.host.deliver(this.peerId, { m: "event", data: e }); }
}
const GRACE_MS = 30000; // reconnection grace window (spec §20.5)
export class GameHost {
    ...
    onPeerMessageObject(peerId, msg) {
        const peer = this.peers.get(peerId);
        if (!peer)
            return;
        if (!peer.helloReceived) {
            if (msg.m !== "hello") {
                this.sink.disconnect(peerId);
                return;
            }
        ...
        if (peer.session)
            this.handleClientMsg(peer.session, msg);
    }
    ...
    deliver(peerId, msg) { this.sink.send(peerId, msg); }
    sessions() {
        const out = [];
    ...
    handleHello(peer, msg) {
        ...
        if (token && this.graceTokens.has(token)) {
            ...
            this.graceTokens.delete(token);
            const session = { peerId: peer.peerId, slotIndex: grace.slotIndex, token, name, link: null };
            peer.session = session;
            if (grace.slotIndex === 0)
                this.hostPeerId = peer.peerId;
            ...
            if (this.matchHost) {
                ...
                for (const c of this.sessions()) {
                    if (c !== session)
                        this.deliver(c.peerId, { m: "event", data: { e: "toast", key: "net.joined", kind: "ok", params: { name } } });
                }
            }
    ...
    handleClientMsg(session, msg) {
        switch (msg.m) {
            case "hello": break; // duplicate — ignore
            case "lobby":
                if (!this.lobby.state.started)
                    this.handleLobbyAction(session, msg.action);
                break;
            case "cmd":
                if (this.matchHost && session.link)
                    this.matchHost.submit(msg.data);
                break;
            case "ping":
    ...
    handleLobbyAction(session, action) {
        const isHost = session.slotIndex === 0;
        switch (action.a) {
            case "setColor":
                this.lobby.setColor(session.slotIndex, action.color);
                break;
            case "setHero":
                this.lobby.setHero(session.slotIndex, action.hero);
                break;
            case "ready":
                this.lobby.setReady(session.slotIndex, action.ready);
                break;
            case "setName":
            ...
            case "removeSlot":
                if (isHost)
                    this.lobby.removeSlot(action.index);
                break;
            case "openSlot":
                if (isHost)
                    this.lobby.openSlot(action.index);
                break;
            case "closeSlot":
                if (isHost)
                    this.lobby.closeSlot(action.index);
                break;
            case "kick":
                if (isHost) {
                    for (const c of this.sessions())
                        if (c.slotIndex === action.index)
                            this.sink.disconnect(c.peerId);
                    this.lobby.kick(action.index);
                }
                break;
            case "setSplit":
                if (isHost)
                    this.lobby.setSplit(action.on);
                break;
            case "start":
                if (isHost && this.lobby.canStart())
                    this.startMatch();
                break;
        }
        this.broadcastLobby();
    ...
    startMatch() {
        ...
        this.matchInterval = setInterval(() => {
            if (!this.matchHost)
                return;
            this.matchHost.step();
            if (this.matchHost.world.winner !== -2)
                setTimeout(() => this.stopMatch(), 5000);
        }, TICK_DT * 1000);
        this.broadcastLobby();
        this.onMatchStart?.();
    }
    stopMatch() {
        ...
        for (const s of this.lobby.state.slots) {
