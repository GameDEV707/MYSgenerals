...
export class Renderer {
    constructor(canvas, world, fx) {
        this.cam = { x: 0, y: 0, zoom: 24 };
        this.W = 0;
        this.H = 0; // viewport size (CSS px)
        this.vx = 0;
        this.vy = 0; // viewport top-left offset on the canvas (CSS px)
        this.visTimer = 0;
        this.smokeTimer = 0;
    ...
    centerOn(x, y) { this.cam.x = x - this.W / this.cam.zoom / 2; this.cam.y = y - this.H / this.cam.zoom / 2; this.clampCam(); }
    clampCam() {
            ...
            for (const e of this.world.entities) {
                if (e.kind === "building" && !e.stub && !e.constructing && e.hp < e.maxHp * 0.66 && this.isVisible(e.pos.x, e.pos.y)) {
                    ...
                    if (heavy && Math.random() < 0.5)
                        this.fx.particles.push({ x: e.pos.x + (Math.random() - 0.5), y: e.pos.y, vx: 0, vy: -0.3, age: 0, life: 0.5, size: 3, color: "#ff8a3c", grav: -0.3 });
                }
            }
        }
    }
    draw() {
        ...
        ctx.beginPath();
        ctx.rect(this.vx, this.vy, this.W, this.H);
        ctx.clip();
        const off = this.fx.shakeOffset();
        ctx.translate(off.x, off.y);
        ctx.fillStyle = "#0a0d10";
        ctx.fillRect(this.vx - 20, this.vy - 20, this.W + 40, this.H + 40);
        this.drawTerrain();
    ...
    drawPointerHint() {
        if (!this.pointerHint)
            return;
        const ctx = this.ctx;
        const x = this.pointerHint.x, y = this.pointerHint.y;
        ctx.save();
    ...
    drawVirtualCursor() {
        if (!this.virtualCursor)
            return;
        const ctx = this.ctx;
        const x = this.virtualCursor.x, y = this.virtualCursor.y;
        ctx.save();
    ...
    drawTerrain() {
        ...
        for (let y = y0; y < y1; y++)
            for (let x = x0; x < x1; x++) {
                ...
                if (t === 2) {
                    ...
                    ctx.fillRect(this.toX(x), this.toY(y) + Math.sin((this.world.time + x + y) * 2) * 1, z + 1, 2);
                }
            }
        ...
        for (let x = x0; x <= x1; x++) {
            ctx.beginPath();
            ctx.moveTo(this.toX(x), this.toY(y0));
            ctx.lineTo(this.toX(x), this.toY(y1));
            ctx.stroke();
        }
        for (let y = y0; y <= y1; y++) {
            ctx.beginPath();
            ctx.moveTo(this.toX(x0), this.toY(y));
            ctx.lineTo(this.toX(x1), this.toY(y));
            ctx.stroke();
        }
        for (const d of m.deposits) {
            if (!this.isVisible(d.x, d.y) && !this.explored[d.y * m.w + d.x])
                continue;
            ...
            for (let i = 0; i < 5; i++) {
                const a = i * 1.3;
                ctx.beginPath();
                ctx.arc(this.toX(d.x + 0.5 + Math.cos(a) * 0.8), this.toY(d.y + 0.5 + Math.sin(a) * 0.8), z * 0.12, 0, Math.PI * 2);
                ctx.fill();
            }
            ctx.globalAlpha = 1;
        }
    ...
    drawBuilding(e) {
        ...
        if (def.weapon && !e.constructing) {
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.moveTo(this.toX(e.pos.x), this.toY(e.pos.y));
    ...
    drawOutpost(e, x, y, z) {
        ...
        for (let i = 0; i < 4; i++) {
            ctx.fillRect(x - s / 2 + i * s / 4 + s * 0.04, y - s / 2 - s * 0.1, s * 0.16, s * 0.16);
        }
        // garrison turret
        ctx.strokeStyle = "#cfd8e0";
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(x + Math.cos(e.turret) * s * 0.45, y + Math.sin(e.turret) * s * 0.45);
        ctx.stroke();
        ...
        ctx.stroke();
        ctx.fillStyle = col;
        ctx.fillRect(x, y - s * 0.95, s * 0.3, s * 0.18);
        if (e.captureProgress > 0) {
            ctx.strokeStyle = this.teamColor(e.captureOwner);
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(x, y, s * 0.66, -Math.PI / 2, -Math.PI / 2 + e.captureProgress * Math.PI * 2);
            ctx.stroke();
        }
        if (this.selection.has(e.id))
            this.drawSelection(e, s * 0.6);
    ...
    drawInfantryShape(e, sh, x, y, r, col) {
        const ctx = this.ctx, z = this.cam.zoom;
        ...
        const a0 = ctx.globalAlpha;
        if (!sh.combat)
            ctx.globalAlpha = a0 * 0.82; // muted tint for workers
        ctx.fillStyle = col;
        if (sh.body === "roundsquare") {
            this.roundRect(x - br, y - br, br * 2, br * 2, br * 0.6);
            ctx.fill();
            ctx.stroke();
        }
        else {
        ...
        if (sh.arm === "core") {
            ...
            ctx.fillRect(x - br * 0.4, y - br * 0.4, br * 0.8, br * 0.8);
            ctx.fillStyle = "#e6edf3";
            ctx.fillRect(x - br * 0.28, y - br * 0.18, br * 0.18, br * 0.18);
            ctx.fillRect(x + br * 0.1, y - br * 0.18, br * 0.18, br * 0.18);
            ctx.strokeStyle = "#cfd8e0";
            ctx.lineWidth = 1.2;
            ctx.beginPath();
            ctx.moveTo(x - br * 0.3, y - br);
            ctx.lineTo(x - br * 0.5, y - br * 1.5);
            ctx.moveTo(x + br * 0.3, y - br);
            ctx.lineTo(x + br * 0.5, y - br * 1.5);
            ctx.stroke();
        }
        if (sh.arm === "rifle") {
            ...
            ctx.beginPath();
            ctx.moveTo(x, y);
            ctx.lineTo(x + Math.cos(e.turret) * br * 1.25, y + Math.sin(e.turret) * br * 1.25);
            ctx.stroke();
        }
            ...
            ctx.lineWidth = Math.max(2.5, r * 0.5);
            ctx.beginPath();
            ctx.moveTo(x + ox - Math.cos(e.turret) * br * 0.4, y + oy - Math.sin(e.turret) * br * 0.4);
            ctx.lineTo(x + ox + Math.cos(e.turret) * br * 1.5, y + oy + Math.sin(e.turret) * br * 1.5);
            ctx.stroke();
        }
        if (sh.glyph) {
            ctx.fillStyle = "#0b0f14";
    ...
    drawVehicleShape(e, sh, x, y, r, col) {
        ...
        if (sh.arm === "single") {
            ...
            ctx.beginPath();
            ctx.moveTo(0, -br * 0.22);
            ctx.lineTo(L, -br * 0.22);
            ctx.moveTo(0, br * 0.22);
            ctx.lineTo(L, br * 0.22);
            ctx.stroke();
        }
            ...
            ctx.fillRect(br * 0.2, -br * 0.55, br * 0.9, br * 1.1);
            ctx.strokeStyle = "#0b0f14";
            ctx.lineWidth = 1;
            ctx.strokeRect(br * 0.2, -br * 0.55, br * 0.9, br * 1.1);
            ctx.fillStyle = "#2b333c";
            for (let i = 0; i < 3; i++) {
                const ty = -br * 0.34 + i * br * 0.34;
                ctx.fillRect(br * 0.5, ty - br * 0.06, br * 0.7, br * 0.12);
            }
        }
        else if (sh.arm === "twin") {
            ctx.lineWidth = Math.max(2, r * 0.3);
            ctx.beginPath();
            ctx.moveTo(0, -br * 0.18);
            ctx.lineTo(L * 0.7, -br * 0.5);
            ctx.moveTo(0, br * 0.18);
            ctx.lineTo(L * 0.7, -br * 0.16);
            ctx.stroke();
        }
        ctx.restore();
        ctx.fillStyle = "#cfd8e0";
        ctx.beginPath();
        ctx.arc(x, y, br * 0.5, 0, Math.PI * 2);
        ctx.fill();
        if (sh.dish) {
            ctx.strokeStyle = "#9fb0bf";
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.arc(x, y, br * 0.32, Math.PI * 0.1, Math.PI * 1.1);
            ctx.stroke();
        }
    }
    drawStrikesAndBanners() {
        const ctx = this.ctx, z = this.cam.zoom;
        for (const s of this.world.strikes) {
            ...
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.arc(x, y, s.radius * z * (0.5 + k * 0.5), 0, Math.PI * 2);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(x - z, y);
            ctx.lineTo(x + z, y);
        ...
        for (const b of this.world.banners) {
            ...
            ctx.lineTo(x, y - z);
            ctx.stroke();
            ctx.fillStyle = this.teamColor(b.owner);
            ctx.fillRect(x, y - z, z * 0.5, z * 0.3);
        }
    }
    drawPlacement() {
        if (!this.placing)
        ...
        ctx.globalAlpha = 1;
        ctx.strokeStyle = ok ? "#34d399" : "#ef4444";
        ctx.lineWidth = 2;
    ...
    drawRadarDish(cx, cy, half) {
        ...
        ctx.beginPath();
        ctx.moveTo(cx, cy);
        ctx.arc(cx, cy, half * 1.7, a, a + Math.PI / 3);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(cx, cy, half, a + Math.PI * 0.6, a + Math.PI * 1.4);
        ctx.stroke();
        // dish mast + hub
        ctx.strokeStyle = "#cfd8e0";
        ctx.lineWidth = 2;
    ...
    drawRallyFlag(rx, ry, bx, by) {
        ...
        ctx.setLineDash([4, 4]);
        ctx.beginPath();
        ctx.moveTo(this.toX(bx), this.toY(by));
        ctx.lineTo(fx, fy);
        ctx.stroke();
        ctx.setLineDash([]);
        // pole
        ...
        for (let y = 0; y < m.h; y++)
            for (let x = 0; x < m.w; x++) {
                ...
                if (!ex) {
                    ...
                    ctx.fillStyle = TERRAIN_COLORS[m.terrain[y * m.w + x]];
                }
