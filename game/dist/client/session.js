...
class Bundle {
    constructor(playerId, primary, view, fx, r, input, hud, loop, root) {
        this.pending = [];
...
export class MatchSession {
    ...
    start(cfg) {
        ...
        sorted.forEach((p, i) => { if (p.isAI)
            this.host.addAIPlayer(i); });
        const locals = cfg.locals.map((ls) => ({ ...ls, playerId: idMap.get(ls.playerId) ?? 0 }));
    ...
    get paused() { return this.bundles.some((b) => b.input.paused); }
    loop(now) {
        if (!this.running)
            return;
