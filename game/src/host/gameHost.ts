...
export class GameHost {
  ...
  private handleLobbyAction(session: PeerSession, action: LobbyAction): void {
    ...
    switch (action.a) {
      ...
      case "setTeam": this.lobby.setTeam(session.slotIndex, action.team); break;
      // host-only:
      case "setMap": if (isHost) this.lobby.setMap(action.map); break;
      case "addAI": if (isHost) this.lobby.addAI(action.diff); break;
      case "addAITeam": if (isHost) this.lobby.addAITeam(action.team, action.diff); break;
      case "setGameType": if (isHost) this.lobby.setGameType(action.gameType); break;
      case "setTeamColor": if (isHost) this.lobby.setTeamColor(action.team, action.color); break;
