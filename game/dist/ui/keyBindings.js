            ...
            for (const ctx of CONTEXTS) {
                ...
                if (src && typeof src === "object") {
                    for (const a of Object.keys(d[ctx])) {
                        if (typeof src[a] === "string" && src[a])
                            d[ctx][a] = src[a].toLowerCase();
                    }
                }
            }
        }
...
export function saveKeyBindings(b) {
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(b));
    }
    catch { /* ignore */ }
}
export function clearKeyBindings() {
    try {
...
export function findConflict(b, context, action, key) {
    const ctx = b[context];
    for (const a of Object.keys(ctx)) {
        if (a !== action && ctx[a] === key)
            return a;
    }
    return null;
}
...
let current = loadKeyBindings();
const listeners = new Set();
function notify() { listeners.forEach((fn) => fn()); }
export function getKeyBindings() { return current; }
export function onBindingsChange(fn) { listeners.add(fn); }
export function reloadKeyBindings() { current = loadKeyBindings(); notify(); }
// Assign a key. Returns the conflicting action name if blocked (same-context duplicate), else null.
export function setBinding(context, action, key) {
...
export function resetKeyBindings(context) {
    const def = defaultKeyBindings();
    if (context)
        current[context] = def[context];
    else
        current = def;
    saveKeyBindings(current);
    notify();
