...
export class Entity {
    constructor(id, kind, type, owner, pos) {
        ...
        this.minerSlots = 0; // miners assigned (silver mine)
        this.power = 0;
        // neutral
...
export class World {
    ...
    tryBuild(cmd) {
        ...
        if (!this.canAfford(p, def.cost)) {
            this.events.push({ e: "toast", key: this.shortfallKey(p, def.cost), kind: "danger", to: cmd.owner });
            return;
        }
        if (def.requires && !this.entities.some((e) => e.owner === cmd.owner && e.type === def.requires && !e.constructing)) {
            this.events.push({ e: "toast", key: "errors.needBuilding", kind: "danger", params: { b: BUILDING_DEFS[def.requires].nameKey }, to: cmd.owner });
            return;
        }
        if (!this.placementValid(cmd.owner, cmd.building, cmd.x, cmd.y)) {
            this.events.push({ e: "toast", key: "errors.invalidPlacement", kind: "danger", to: cmd.owner });
            return;
        }
        ...
        if (demand > 0) {
            ...
            for (const e of this.entities) {
                if (!e.dead && e.kind === "building" && e.owner === cmd.owner && e.constructing && e.power < 0)
                    committed += -e.power;
            }
            if (committed + demand > p.powerGen) {
                this.events.push({ e: "toast", key: "errors.needPower", kind: "danger", to: cmd.owner });
                return;
            }
        }
        this.pay(p, def.cost);
    ...
    tryUpgradeBuilding(buildingId, kind) {
        ...
        const def = BUILDING_DEFS[b.type];
        if (!def.produces)
            return; // only producing buildings can be upgraded
        const p = this.players[b.owner];
        if (!p)
            return;
        if (kind === "bay") {
    ...
    cancelResearch(buildingId) {
        ...
        if (def)
            this.refund(this.players[b.owner], def.cost, CANCEL_INPROGRESS_REFUND);
        b.researching = null;
    }
    completeResearch(b) {
        if (!b.researching)
    ...
    economySystem() {
        ...
        for (const e of this.entities) {
            ...
            if (e.type === "command_center")
                p.powerGen += 0; // base already +5 via def power
        }
        for (const p of this.players)
            p.brownout = (p.powerGen - p.powerUse) < 0;
    ...
    workerSystem() {
        ...
        for (const e of this.entities)
            if (e.type === "silver_mine" && !e.dead)
                e.minerSlots = this.countMiners(e.id);
    }
    countMiners(mineId) {
        let n = 0;
        for (const e of this.entities)
    ...
    autoAssignMiner(m) {
        ...
        let bd = 1e9;
        for (const e of this.entities) {
            if (e.type !== "silver_mine" || e.dead || e.owner !== m.owner || e.constructing)
                continue;
            if (this.countMiners(e.id) >= SILVER_MINE_SLOTS)
                continue;
