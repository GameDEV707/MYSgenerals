// MYS Generals — FxRenderer (spec §16). Pooled, recycled visual effects fed by sim events.
import { Vec2 } from "../types.js";
import { GameEvent } from "../sim/world.js";

export type Quality = "low" | "med" | "high";

interface Particle { x: number; y: number; vx: number; vy: number; age: number; life: number; size: number; color: string; grav: number; }
interface Explosion { x: number; y: number; age: number; life: number; size: number; kind: string; }
interface Muzzle { x: number; y: number; angle: number; age: number; }
interface FloatText { x: number; y: number; text: string; color: string; age: number; }
...
// Cosmetic command marker: immediate visual feedback at the click target (spec §20.4 prediction).
interface CmdMarker { x: number; y: number; age: number; color: string; kind: "move" | "attack"; }
...
const CAPS: Record<Quality, typeof CAP> = {
...
export class FxRenderer {
  ...
  quality: Quality = "med";
  reduceMotion = false;
  onToast?: (key: string, kind: string | undefined, params?: Record<string, string | number>) => void;
  onPing?: (x: number, y: number, color: string) => void;

  private qScale(): number { return this.quality === "high" ? 1.4 : this.quality === "low" ? 0.5 : 1; }
  ...
  addCmdMarker(x: number, y: number, kind: "move" | "attack", color: string): void {
    this.cmdMarkers.push({ x, y, age: 0, color, kind });
    if (this.cmdMarkers.length > 20) this.cmdMarkers.shift();
  }

  consume(ev: GameEvent, teamColor: (owner: number) => string): void {
    switch (ev.e) {
      case "fire": {
      ...
      case "capture": this.burst(ev.pos, 24, teamColor(ev.owner), 1.0); this.pings.push({ x: ev.pos.x, y: ev.pos.y, age: 0, color: teamColor(ev.owner) }); break;
      case "rankup": this.texts.push({ x: ev.pos.x, y: ev.pos.y, text: "▲", color: "#ffd23f", age: 0 }); this.burst(ev.pos, 8, "#ffd23f", 0.6); break;
      case "shake": if (!this.reduceMotion) this.shake = Math.max(this.shake, ev.intensity); break;
      case "flash": if (!this.reduceMotion) this.flash = { color: ev.color, a: 1 }; break;
      case "ability": this.ability(ev.slot, ev.pos, teamColor(ev.owner)); break;
      case "toast": if (this.onToast) this.onToast(ev.key, ev.kind, ev.params); break;
    }
  }

  private ability(slot: number, pos: Vec2, color: string): void {
    if (slot < 0) return; // denied (handled by UI shake)
    if (slot === 1) this.burst(pos, 20, "#ffd23f", 1.2); // banner
    else if (slot === 2) this.burst(pos, 24, "#cfd8e0", 1.4); // roll shock
    else this.burst(pos, 16, color, 1.0);
  }

  private explode(pos: Vec2, kind: string, size: number): void {
    ...
    if (size >= 1) this.decals.push({ x: pos.x, y: pos.y, age: 0, life: 8, size: size * 0.7 });
    this.capArrays();
  }

  private death(pos: Vec2, kind: string, color: string): void {
    if (kind === "infantry") this.burst(pos, 8, "#b04a3a", 0.5);
    else if (kind === "vehicle") { this.explode(pos, "shell", 1.2); this.burst(pos, 18, "#444", 1.0); }
    else if (kind === "building") { this.explode(pos, "artillery", 2.0); this.burst(pos, 30, "#777", 1.6); this.decals.push({ x: pos.x, y: pos.y, age: 0, life: 12, size: 2 }); }
    this.capArrays();
  }

  burst(pos: Vec2, count: number, color: string, spread: number): void {
  ...
  private capArrays(): void {
    ...
    if (this.particles.length > c.particles) this.particles.splice(0, this.particles.length - c.particles);
    if (this.explosions.length > c.explosions) this.explosions.splice(0, this.explosions.length - c.explosions);
    if (this.decals.length > c.decals) this.decals.splice(0, this.decals.length - c.decals);
    if (this.texts.length > c.texts) this.texts.splice(0, this.texts.length - c.texts);
    if (this.tracers.length > c.tracers) this.tracers.splice(0, this.tracers.length - c.tracers);
    if (this.projectiles.length > c.projectiles) this.projectiles.splice(0, this.projectiles.length - c.projectiles);
  }

  update(dt: number): void {
    for (const p of this.particles) { p.age += dt; p.x += p.vx * dt; p.y += p.vy * dt; p.vy += p.grav * dt; }
    ...
    for (const tr of this.tracers) tr.age += dt;
    this.tracers = this.tracers.filter((tr) => tr.age < 0.08);
    for (const t of this.texts) { t.age += dt; t.y -= dt * 0.8; }
  ...
  drawGround(ctx: CanvasRenderingContext2D, toX: (wx: number) => number, toY: (wy: number) => number, z: number): void {
    // command markers (cosmetic prediction, spec §20.4)
    for (const cm of this.cmdMarkers) {
      const k = cm.age / 0.6;
      const r = z * (0.3 + k * 0.5);
      ctx.globalAlpha = (1 - k) * 0.7;
      ctx.strokeStyle = cm.color; ctx.lineWidth = cm.kind === "attack" ? 2.5 : 1.8;
      ctx.beginPath(); ctx.arc(toX(cm.x), toY(cm.y), r, 0, Math.PI * 2); ctx.stroke();
      if (cm.kind === "attack") {
        // cross for attack
        const s = r * 0.4;
        ctx.beginPath(); ctx.moveTo(toX(cm.x) - s, toY(cm.y) - s); ctx.lineTo(toX(cm.x) + s, toY(cm.y) + s); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(toX(cm.x) + s, toY(cm.y) - s); ctx.lineTo(toX(cm.x) - s, toY(cm.y) + s); ctx.stroke();
      }
    }
    ctx.globalAlpha = 1;
    ...
    for (const d of this.decals) {
      ...
      ctx.beginPath(); ctx.ellipse(toX(d.x), toY(d.y), d.size * z, d.size * z * 0.6, 0, 0, Math.PI * 2); ctx.fill();
    }
  }

  // overhead effects drawn above entities
  draw(ctx: CanvasRenderingContext2D, toX: (wx: number) => number, toY: (wy: number) => number, z: number): void {
    // tracers
    ctx.lineWidth = 2;
    for (const tr of this.tracers) {
      ctx.strokeStyle = tr.color; ctx.globalAlpha = 1 - tr.age / 0.08;
      ctx.beginPath(); ctx.moveTo(toX(tr.x1), toY(tr.y1)); ctx.lineTo(toX(tr.x2), toY(tr.y2)); ctx.stroke();
    }
    ctx.globalAlpha = 1;
    // muzzle flashes
    for (const m of this.muzzles) {
      ...
      ctx.beginPath(); ctx.arc(x + Math.cos(m.angle) * z * 0.4, y + Math.sin(m.angle) * z * 0.4, z * 0.22, 0, Math.PI * 2); ctx.fill();
    }
    // particles
    ...
    for (const p of this.projectiles) {
      ...
      if (p.kind === "rocket" || p.kind === "flak") {
        ...
        ctx.fillStyle = "#ddd"; ctx.fillRect(-z * 0.2, -z * 0.08, z * 0.4, z * 0.16);
        ctx.fillStyle = "#ff8a3c"; ctx.fillRect(-z * 0.3, -z * 0.05, z * 0.12, z * 0.1); ctx.restore();
      } else if (p.kind === "shell") {
        ctx.fillStyle = "#ffd27a"; ctx.beginPath(); ctx.arc(x, y, z * 0.1, 0, Math.PI * 2); ctx.fill();
      } else if (p.kind === "energy") {
        ctx.fillStyle = "#7ad7ff"; ctx.globalAlpha = 0.9; ctx.beginPath(); ctx.arc(x, y, z * 0.15, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1;
      } else if (p.kind === "artillery") {
        const k = p.total > 0 ? p.t / p.total : 1; const h = Math.sin(k * Math.PI) * z * 1.6;
        ctx.fillStyle = "rgba(0,0,0,0.3)"; ctx.beginPath(); ctx.ellipse(x, y, z * 0.18, z * 0.1, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#bbb"; ctx.beginPath(); ctx.arc(x, y - h, z * 0.12, 0, Math.PI * 2); ctx.fill();
      } else {
        ctx.fillStyle = "#ffd27a"; ctx.beginPath(); ctx.arc(x, y, z * 0.1, 0, Math.PI * 2); ctx.fill();
      }
    }
    ...
    for (const e of this.explosions) {
      const k = e.age / e.life; const r = e.size * z * (0.3 + k * 1.1);
      const x = toX(e.x), y = toY(e.y);
      ctx.globalAlpha = (1 - k) * 0.8;
      const grad = ctx.createRadialGradient(x, y, 0, x, y, r);
      grad.addColorStop(0, "rgba(255,240,180,0.95)"); grad.addColorStop(0.5, "rgba(255,140,40,0.7)"); grad.addColorStop(1, "rgba(120,40,10,0)");
      ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill();
      // shockwave ring
      ctx.globalAlpha = (1 - k) * 0.5; ctx.strokeStyle = "rgba(255,255,255,0.8)"; ctx.lineWidth = 2;
    ...
    ctx.globalAlpha = 1;
    // floating text
    ctx.textAlign = "center"; ctx.font = `bold ${Math.max(11, z * 0.6)}px Noto Sans, sans-serif`;
    for (const t of this.texts) {
      ctx.globalAlpha = Math.max(0, 1 - t.age / 0.8);
      ctx.fillStyle = "rgba(0,0,0,0.6)"; ctx.fillText(t.text, toX(t.x) + 1, toY(t.y) + 1);
      ctx.fillStyle = t.color; ctx.fillText(t.text, toX(t.x), toY(t.y));
    }
    ctx.globalAlpha = 1;
  }
  ...
  drawFlash(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    ...
    ctx.fillStyle = this.flash.color; ctx.fillRect(0, 0, w, h); ctx.globalAlpha = 1;
  }
}
