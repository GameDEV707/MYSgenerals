...
function setter(terrain, w, h) {
    return (x, y, v) => { if (x >= 0 && y >= 0 && x < w && y < h)
        terrain[y * w + x] = v; };
}
...
function fortifyBase(terrain, w, h, sx, sy) {
    ...
    for (let y = sy - R; y <= sy + R; y++) {
        if (Math.abs(y - sy) <= GATE)
            continue;
        set(innerX, y, WALL);
    }
    for (let x = sx - R; x <= sx + R; x++) {
        if (Math.abs(x - sx) <= GATE)
            continue;
        set(x, innerY, WALL);
    }
...
function baseDeposits(sx, sy, cx, cy) {
...
function laneRoad(set, x0, y0, x1, y1, width = 2) {
    ...
    for (let i = 0; i <= steps; i++) {
        ...
        for (let dy = -half; dy <= width - 1 - half; dy++)
            for (let dx = -half; dx <= width - 1 - half; dx++)
                set(cx + dx, cy + dy, ROAD);
    }
}
...
function wallArc(set, cx, cy, radius, gateAnglesDeg, gateHalfWidthDeg = 12, tile = WALL) {
    for (let a = 0; a < 360; a += 2) {
        let inGate = false;
        for (const g of gateAnglesDeg) {
            const d = Math.abs(((a - g + 540) % 360) - 180);
            if (d <= gateHalfWidthDeg)
                inGate = true;
        }
        if (inGate)
            continue;
...
function clearObjects(terrain, w, h, deposits, neutrals) {
    const set = setter(terrain, w, h);
    const cx = Math.floor(w / 2), cy = Math.floor(h / 2);
    for (let dy = -1; dy <= 1; dy++)
        for (let dx = -1; dx <= 1; dx++)
            set(cx + dx, cy + dy, GRASS);
    for (const d of deposits)
        set(d.x, d.y, GRASS);
    for (const n of neutrals) {
        const ap = n.kind === "fortress" ? 5 : 2;
        for (let dy = -ap; dy <= ap; dy++)
            for (let dx = -ap; dx <= ap; dx++)
                set(n.x + dx, n.y + dy, GRASS);
    }
}
// ───────────────────────────────────────────────────────────────────────────────────────────────
// A1 — "Twin Spear" — 2 players — 104×64 (fast triple-lane duel; 1 central fortress).
export function buildTwinSpear() {
    const w = 104, h = 64;
    ...
    const spawns = [{ x: 14, y: 18 }, { x: 89, y: 45 }];
    for (const s of spawns)
        fortifyBase(terrain, w, h, s.x, s.y);
...
export function buildQuadFoundry() {
    const w = 112, h = 112;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 56, cy = 56;
    ...
    const spawns = [{ x: 16, y: 16 }, { x: 95, y: 95 }, { x: 95, y: 16 }, { x: 16, y: 95 }];
    for (const s of spawns)
        fortifyBase(terrain, w, h, s.x, s.y);
    // a cross of roads from each gate through the edge-mid outposts toward the ramps
    laneRoad(set, 56, 18, 56, 40, 2);
    laneRoad(set, 56, 72, 56, 94, 2);
    laneRoad(set, 18, 56, 40, 56, 2);
    laneRoad(set, 72, 56, 94, 56, 2);
    laneRoad(set, 24, 24, 42, 42, 2);
    laneRoad(set, 88, 88, 70, 70, 2);
    laneRoad(set, 88, 24, 70, 42, 2);
    laneRoad(set, 24, 88, 42, 70, 2);
...
export function buildSerpentDelta() {
    const w = 116, h = 100;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 58, cy = 50;
    ...
    for (const by of [20, 50, 80])
        laneRoad(set, 50, by, 66, by, 3);
    // bridge over the tributary
    laneRoad(set, 36, 70, 47, 70, 3);
    const spawns = [{ x: 16, y: 24 }, { x: 16, y: 76 }, { x: 99, y: 24 }, { x: 99, y: 76 }];
    for (const s of spawns)
        fortifyBase(terrain, w, h, s.x, s.y);
...
export function buildHexBazaar() {
    const w = 132, h = 116;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 66, cy = 58;
...
export function buildIronOctagon() {
    const w = 148, h = 148;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 74, cy = 74;
    ...
    for (let a = 0; a < 360; a += 45)
        ringPts.push([Math.round(cx + Math.cos(a * Math.PI / 180) * 44), Math.round(cy + Math.sin(a * Math.PI / 180) * 44)]);
...
export function buildNecrokeepLine() {
    const w = 168, h = 88;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 84, cy = 44;
    ...
    for (let x = 40; x <= 70; x += 1) {
        set(x, 27, WALL);
        set(x, 61, WALL);
    }
    for (let x = 98; x <= 128; x += 1) {
        set(x, 27, WALL);
        set(x, 61, WALL);
    }
...
export function buildAshfallCrucible() {
    const w = 116, h = 116;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 58, cy = 58;
    ...
    for (let y = 0; y < h; y++)
        for (let x = 0; x < w; x++) {
            ...
            if (d >= 14 && d <= 18)
                set(x, y, WATER); // lava moat
            else if (d > 18 && d <= 20)
                set(x, y, CLIFF); // caldera rim
        }
    const spawns = [{ x: 54, y: 16 }, { x: 20, y: 96 }, { x: 96, y: 96 }];
    // three land bridges (road) radiating from the central courtyard to each base
    for (const s of spawns)
        laneRoad(set, cx, cy, s.x, s.y, 3);
    for (const s of spawns)
        fortifyBase(terrain, w, h, s.x, s.y);
