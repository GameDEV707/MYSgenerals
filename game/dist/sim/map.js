...
export function buildTwinRivers() {
    ...
    const set = (x, y, v) => { if (x >= 0 && y >= 0 && x < w && y < h)
        terrain[y * w + x] = v; };
    ...
    const spawns = [{ x: 8, y: 8 }, { x: 56, y: 56 }];
    const deposits = [
...
export function buildCrossfire() {
    ...
    for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]]) {
        for (let k = 8; k <= 12; k++) {
            set(cx + dx * k, cy + dy * k, 0);
            set(cx + dx * k + (dy !== 0 ? 1 : 0), cy + dy * k + (dx !== 0 ? 1 : 0), 0);
        }
    }
    const spawns = [{ x: 8, y: 8 }, { x: 63, y: 63 }, { x: 63, y: 8 }, { x: 8, y: 63 }];
    const deposits = [
