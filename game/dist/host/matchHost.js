...
export class MatchHost {
    ...
    economyOwner(pid) {
        ...
        for (const e of this.world.entities) {
            if (e.dead || e.type !== "command_center")
                continue;
            if (mem.includes(e.owner) && (best < 0 || e.owner < best))
                best = e.owner;
        }
    ...
    buildSnapshot(pid, grid) {
        ...
        const isMine = (owner) => teammates.has(owner); // ally-owned counts as "own": full detail
        const eco = w.players[this.economyOwner(pid)]; // the side's shared base owner (economy source)
        const mem = this.memory.get(pid) ?? new Map();
        const entities = [];
        ...
        const players = w.players.map((p) => {
            ...
            if (p.id === pid) {
                ...
                base.silver = eco.silver;
                base.iron = eco.iron;
                base.gold = eco.gold;
                base.powerGen = eco.powerGen;
                base.powerUse = eco.powerUse;
                base.brownout = eco.brownout;
                ...
                base.unitsBuilt = eco.unitsBuilt;
                base.unitsLost = eco.unitsLost;
                base.buildingsDestroyed = eco.buildingsDestroyed;
            }
        ...
        for (const b of w.banners) {
            if (teammates.has(b.owner) || this.visAt(grid, b.pos.x, b.pos.y))
    ...
    snapEntity(e, mine) {
        ...
        if (mine) {
            ...
            if (!e.constructing) {
                ...
                if (eta)
                    s.mn = { s: eta.seconds == null ? 0 : eta.seconds, p: eta.progress, res: eta.resource, idle: eta.idle, free: !isOil && this.world.claimedMiners(e.id) < mineSlotCap(e.type) };
            }
            if (e.hero)
