...
function rsEncode(data, ecLen) {
    ...
    for (let i = 0; i < ecLen; i++) {
        for (let j = gen.length; j > 0; j--)
            gen[j] = (gen[j] ?? 0) ^ gfMul(gen[j - 1], EXP[i]);
...
function makePatterns(size, v) {
    const m = Array.from({ length: size }, () => new Array(size).fill(null));
    const reserved = Array.from({ length: size }, () => new Array(size).fill(false));
    const set = (r, c, val) => { m[r][c] = val; reserved[r][c] = true; };
    const finder = (r0, c0) => {
        for (let r = -1; r <= 7; r++)
            for (let c = -1; c <= 7; c++) {
...
function applyMask(val, r, c, mask) {
    let on = false;
    switch (mask) {
        case 0:
            on = (r + c) % 2 === 0;
        ...
        case 3:
            ...
            break;
        case 4:
            on = (Math.floor(r / 2) + Math.floor(c / 3)) % 2 === 0;
            break;
        case 5:
            on = ((r * c) % 2) + ((r * c) % 3) === 0;
            break;
...
function penalty(grid, size) {
    ...
    for (let r = 0; r < size; r++)
        for (let c = 0; c < size - 10; c++) {
            let a = true, b = true;
            for (let k = 0; k < 11; k++) {
                if (grid[r][c + k] !== pat1[k])
                    a = false;
                if (grid[r][c + k] !== pat2[k])
                    b = false;
            }
            if (a || b)
                p += 40;
            ...
            b = true;
            for (let k = 0; k < 11; k++) {
                if (grid[c + k][r] !== pat1[k])
                    a = false;
                if (grid[c + k][r] !== pat2[k])
                    b = false;
            }
            if (a || b)
                p += 40;
...
export function qrMatrix(text) {
    ...
    for (let v = 1; v <= 10; v++) {
        ...
        if (bytes.length + headerBytes <= dataCapacityBytes(v)) {
            version = v;
            break;
        }
    }
    ...
    // build bit stream
    const bits = [];
    const push = (val, len) => { for (let i = len - 1; i >= 0; i--)
        bits.push((val >> i) & 1); };
    ...
    let off = 0;
    for (let i = 0; i < g1; i++) {
        const data = codewords.slice(off, off + d1);
        off += d1;
        blocks.push({ data, ec: rsEncode(data, ecLen) });
    }
    for (let i = 0; i < g2; i++) {
        const data = codewords.slice(off, off + d2);
        off += d2;
        blocks.push({ data, ec: rsEncode(data, ecLen) });
    }
...
function writeFormat(g, reserved, size, mask, version) {
...
export function qrAscii(m) {
    const size = m.length;
    const q = 2; // quiet zone
    const dark = (r, c) => r >= 0 && r < size && c >= 0 && c < size && m[r][c];
    const lines = [];
