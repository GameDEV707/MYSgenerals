...
function acceptWebSocket(req: IncomingMessage, socket: import("node:net").Socket, head: Buffer): WSConnection | null {
  ...
  const conn: WSConnection = {
    ...
    send(msg: ServerMsg) {
      ...
      try { sendFrame(socket, JSON.stringify(msg)); } catch { this.alive = false; }
    },
    close() {
      this.alive = false;
...
function sendFrame(socket: import("node:net").Socket, data: string): void {
