...
export interface SignalBlob {
  t: SignalKind;       // offer (invite) | answer (reply)
  sdp: string;         // full session description (ICE candidates embedded, non-trickle)
  ice?: string[];      // optional extra candidates (tolerated, normally empty)
...
export function encodeSignal(blob: SignalBlob): string {
  const payload: SignalBlob = { t: blob.t, sdp: blob.sdp, v: blob.v ?? SIGNAL_VERSION };
  if (blob.ice && blob.ice.length) payload.ice = blob.ice;
...
export function decodeSignal(code: string): SignalBlob | null {
  if (!code) return null;
  let raw = code.trim();
  ...
  try {
    ...
    if (!obj || (obj.t !== "offer" && obj.t !== "answer") || typeof obj.sdp !== "string" || !obj.sdp) return null;
    const blob: SignalBlob = { t: obj.t, sdp: obj.sdp, v: typeof obj.v === "number" ? obj.v : SIGNAL_VERSION };
    if (Array.isArray(obj.ice)) blob.ice = obj.ice.filter((x: unknown) => typeof x === "string");
    return blob;
  } catch {
    return null;
  }
