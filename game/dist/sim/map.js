...
export function buildTwinRivers() {
    const w = 80, h = 80;
    ...
    for (const by of [20, 40, 60])
        for (let dy = 0; dy <= 1; dy++)
            for (let x = 35; x <= 44; x++)
                set(x, by + dy, ROAD);
    const spawns = [{ x: 10, y: 10 }, { x: 69, y: 69 }];
...
export function buildCrossfire() {
    const w = 88, h = 88;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 44, cy = 44;
    ...
    const spawns = [{ x: 10, y: 10 }, { x: 77, y: 77 }, { x: 77, y: 10 }, { x: 10, y: 77 }];
    for (const s of spawns)
        fortifyBase(terrain, w, h, s.x, s.y);
...
export function buildIronCrossroads() {
    const w = 96, h = 96;
    const terrain = blank(w, h);
    const set = setter(terrain, w, h);
    const cx = 48, cy = 48;
    ...
    const spawns = [{ x: 12, y: 12 }, { x: 83, y: 83 }, { x: 83, y: 12 }, { x: 12, y: 83 }];
    for (const s of spawns)
        fortifyBase(terrain, w, h, s.x, s.y);
